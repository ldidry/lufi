...
var populateFilesTable = () => {
  ...
  const addDeletedItem = (file) => {
    const deletedItemDOM = document.querySelector("template#deleted-item").content.cloneNode(true).children[0];
    deletedItemDOM.dataset.serverKey = file.short;
    deletedItemDOM.querySelector(".name").innerText = file.name;
    deletedItemDOM.querySelector(".delete-at-first-view .icon").classList.add(file.del_at_first_view ? "fa-eraser" : "fa-close");
    deletedItemDOM.querySelector(".delete-at-first-view .icon").setAttribute("title", file.del_at_first_view ? i18n.yes : i18n.no);
    deletedItemDOM.querySelector(".created-at").innerText = formatDate(
      file.created_at
    );
    deletedItemDOM.querySelector(".expires-at").innerText = file.delay == 0 ? i18n.noExpiration : formatDate(file.delay * 86400 + file.created_at);
  ...
  files.forEach((file) => {
    ...
    itemDOM.querySelector(".delete-at-first-view .icon").setAttribute("title", file.del_at_first_view ? i18n.yes : i18n.no);
